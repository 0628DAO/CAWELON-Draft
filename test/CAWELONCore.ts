...
const INITIAL_SUPPLY_UNITS = 420_000_000_000_000n;
const INITIAL_SUPPLY = INITIAL_SUPPLY_UNITS * 10n ** 18n;

describe("CAWELONCore", async function () {
  ...
  async function deploy() {
    return viem.deployContract("CAWELONCore", [holder.account.address]);
  }
  ...
  it("uses CAWELON CORE as the token name", async function () {
    const token = await deploy();
    assert.equal(await token.read.name(), "CAWELON CORE");
  });

  it("uses CAWELON as the token symbol", async function () {
    const token = await deploy();
    assert.equal(await token.read.symbol(), "CAWELON");
  });
  ...
  it("rejects the zero address as initial holder", async function () {
    await assert.rejects(() => viem.deployContract("CAWELONCore", [zeroAddress]));
  });

  it("transfers CAWELON between holders", async function () {
    const token = await deploy();
  ...
  it("mints to a separate initial holder, not the transaction sender", async function () {
    const token = await viem.deployContract("CAWELONCore", [recipient.account.address]);
  ...
  it("lets a holder burn its own CAWELON", async function () {
    const token = await deploy();
  ...
  it("publishes the expected EIP-712 domain", async function () {
    ...
    assert.equal(domain[1], "CAWELON CORE");
